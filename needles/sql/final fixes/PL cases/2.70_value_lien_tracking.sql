...
CREATE NONCLUSTERED INDEX IX_NonClustered_Index_value_tab_Liencheckbox_Helper_value_id ON [dbo].[value_tab_Liencheckbox_Helper] (value_id);
GO
...
INNER JOIN [SA].[dbo].[sma_TRN_Plaintiff] T
...
INNER JOIN [SA].[dbo].[IndvOrgContacts_Indexed] IOC
	ON IOC.SAGA = V.party_id
INNER JOIN [SA].[dbo].[sma_TRN_Defendants] D
...
---------------------------------------------------------------------------------------
ALTER TABLE [dbo].[sma_TRN_Lienors] DISABLE TRIGGER ALL
ALTER TABLE [dbo].[sma_TRN_LienDetails] DISABLE TRIGGER ALL
...
INSERT INTO [dbo].[sma_TRN_Lienors]
   ...
   ,[saga]
	...
	SELECT
	   ...
	   ,V.value_id				 AS [saga]
	FROM [JoelBieberNeedles].[dbo].[value_Indexed] V
	INNER JOIN [SA].[dbo].[value_tab_Lien_Helper] MAP
