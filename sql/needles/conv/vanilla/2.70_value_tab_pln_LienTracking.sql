...
delete from [TestNeedles].[dbo].[sma_TRN_Lienors] 
DBCC CHECKIDENT ('[TestNeedles].[dbo].[sma_TRN_Lienors]', RESEED, 0);
...
delete from [TestNeedles].[dbo].[sma_TRN_LienDetails] 
DBCC CHECKIDENT ('[TestNeedles].[dbo].[sma_TRN_LienDetails]', RESEED, 0);
...
select count(*) from [TestNeedles].[dbo].[sma_TRN_Lienors]

select * from value_tab_Liencheckbox_Helper 

select * from [TestNeedles].[dbo].[value_payment] where value_id=65990
...
select VP1.value_id
from [TestNeedles].[dbo].[value_payment] VP1 
    left join (
...
---(0)---
insert into [TestNeedles].[dbo].[sma_MST_LienType]
(
    [lntsCode]
    ,[lntsDscrptn]
)
(
    ...
    inner join [TestNeedles].[dbo].[value_code] VC
        on VC.code = V.code 
    where isnull(V.code,'') in (SELECT code FROM #LienValueCodes)
)
except
select [lntsCode],[lntsDscrptn] from [TestNeedles].[dbo].[sma_MST_LienType] 
GO
...
CREATE NONCLUSTERED INDEX IX_NonClustered_Index_value_tab_Lien_Helper_case_id ON [TestNeedles].[dbo].[value_tab_Lien_Helper] (case_id);   
CREATE NONCLUSTERED INDEX IX_NonClustered_Index_value_tab_Lien_Helper_value_id ON [TestNeedles].[dbo].[value_tab_Lien_Helper] (value_id);   
CREATE NONCLUSTERED INDEX IX_NonClustered_Index_value_tab_Lien_Helper_ProviderNameId ON [TestNeedles].[dbo].[value_tab_Lien_Helper] (ProviderNameId);   
GO
...
(
  ...
  select 
    ...
    ( select top 1 lntnLienTypeID FROM [TestNeedles].[dbo].[sma_MST_LienType] where lntsDscrptn=
	   (select [description] FROM [TestNeedles].[dbo].[value_code] where [code]=V.code)) 
						  as [lnrnLienorTypeID],				   
