...
SELECT DISTINCT
  ...
  LEFT JOIN JoelBieberNeedles.[dbo].[user_counsel_data] UD on UD.counsel_id=C.counsel_id and C.case_num=UD.casenum
  JOIN [sma_TRN_Cases] CAS
	on CAS.cassCaseNumber = C.case_num
  JOIN IndvOrgContacts_Indexed IOC
	on IOC.SAGA = C.counsel_id
...
--     ,(
-- 		select atnnAtorneyTypeID
-- 		FROM JoelBieberNeedles..[sma_MST_AttorneyTypes]
-- 		where atnsAtorneyDscrptn='Defense Attorney'
-- 	)									as [lwfnAttorneyTypeID]
--     ,null								as [lwfsFileNumber]
--     ,2									as [lwfnRoleType]
...
-- 	join JoelBieberNeedles..sma_TRN_Cases cas
-- 		on cas.cassCaseNumber = c.casenum
-- 	-- field link
-- 	JOIN JoelBieberNeedles.[dbo].[user_party_name] N 
...
-- 	join JoelBieberNeedles.[dbo].names
...
-- 	join JoelBieberNeedles..sma_TRN_Defendants d
...
FROM (
  ...
  JOIN JoelBieberNeedles.[dbo].[names] L on C.counsel_id = L.names_id
