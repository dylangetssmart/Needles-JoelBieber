...
FROM  [JoelBieberNeedles].[dbo].[mini_general_dir]
...
FROM  [JoelBieberNeedles].[dbo].[user_tab2_data] D
...
FROM [JoelBieberNeedles].[dbo].[user_tab2_data] D
JOIN [JoelBieberNeedles].[dbo].[user_tab2_name] N
	on N.tab_id = D.tab_id
	and N.user_name <> 0
	and N.ref_num = (
						SELECT TOP 1 M.ref_num
						FROM [JoelBieberNeedles].[dbo].[user_tab2_matter] M
...
inner join [JoelBieberNeedles].[dbo].[user_tab2_data] UD on UD.tab_id=A.tab_id
where A.RowNumber=1
*/
...
SELECT 
	...
	,'tab2: '+ convert(varchar,UD.tab_id)			as SAGA
FROM [JoelBieberNeedles].[dbo].[user_tab2_data] UD
JOIN user_tab2_MedicalProvider_Helper MAP
